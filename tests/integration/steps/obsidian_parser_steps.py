...
def step_verify_markdown_count(context, count):
    ...
    assert len(context.result['markdown_files']) == count, \
...
def step_verify_attachment_count(context, count):
    ...
    assert len(context.result['attachments']) == count, \
...
def step_verify_empty_wikilink_map(context):
    ...
    assert len(context.result['wikilink_map']) == 0, \
        f"Expected empty wikilink map, found {len(context.result['wikilink_map'])} entries"
...
def step_verify_wikilinks(context):
    ...
    assert len(wikilinks) == len(context.table.rows), \
...
def step_verify_wikilink_map(context):
    ...
    for row in context.table:
        ...
        assert str(actual_path).endswith(expected_end), \
